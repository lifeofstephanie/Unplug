...
export default function CourseRoadmapScreen() {
  ...
  const startLesson = (lesson) => {
    ...
    setTimeout(() => {
      if (lesson.quiz?.length > 0) {
        router.push({ pathname: "/quiz/[id]", params: { id: lesson._id, courseId, lessonStr: JSON.stringify(lesson) } });
      } else {
        router.push({ pathname: "/lesson/[id]", params: { id: lesson._id, courseId, lessonStr: JSON.stringify(lesson) } });
      }
    }, 250);
  };
  ...
  return (
    <View style={{ flex: 1, backgroundColor: Colors.background }}>
        ...
        <Animated.View style={{ opacity: headerFade }}>
          ...
          <View style={styles.xpBarWrap}>
            ...
            <View style={styles.xpBarBg}>
              <Animated.View
                style={[styles.xpBarFill, { width: `${progress.percent}%` }]}
              />
            </View>
          </View>
        </Animated.View>
      ...
      <ScrollView
        style={{ flex: 1 }}
        contentContainerStyle={[styles.roadmap, { minHeight: totalHeight }]}
