...
export default function HomeScreen() {
  ...
  const inProgressCourses = downloadedList
    .map((id) => {
      ...
      const meta = safeCourses.find((c) => c._id === id);
  ...
  useEffect(() => {
    const loadCourses = async () => {
      try {
        ...
        useStore.getState().setCourses(fetchedCourses);
      } catch (err) {
    ...
    const loadProfile = async () => {
      try {
        ...
        // Update user data, xp, and streak to match backend truth
        useStore.setState((s) => ({ 
          user: me, 
          totalXp: Math.max(s.totalXp || 0, me.xp || 0), 
          streak: Math.max(s.streak || 0, me.streak || 0) 
        }));
      } catch (err) {
