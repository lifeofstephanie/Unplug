...
export default function LessonScreen() {
  ...
  const lesson =
    typeof lessonStr === "string" ? JSON.parse(lessonStr) : lessonStr;
  ...
  const handleComplete = async () => {
    ...
    Animated.sequence([
      Animated.spring(completeBtnScale, {
        toValue: 0.94,
        useNativeDriver: true,
      }),
      Animated.spring(completeBtnScale, {
    ...
    // If lesson has a quiz, proceed to quiz
    if (lesson.quiz?.length > 0) {
      router.replace({
        pathname: "/quiz/[id]",
        params: { id: lesson._id, courseId, lessonStr: JSON.stringify(lesson) },
      });
      return;
    }
    ...
    try {
      ...
      // It's perfectly fine to fail here; we remain in the offline queue
          ...
          <Text key={i} style={styles.para}>
            {parts.map((p, j) => {
              if (p.startsWith("**") && p.endsWith("**")) {
                return (
                  <Text key={j} style={styles.inlineBold}>
                    {p.slice(2, -2)}
                  </Text>
                );
              }
  ...
  return (
    <View style={{ flex: 1, backgroundColor: Colors.background }}>
      ...
      {/* Header */}
      <View style={[styles.header, { paddingTop: 150 }]}>
